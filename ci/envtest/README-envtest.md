...
| `ENVTEST_BIN_DIR` | `_output/tools/bin` | Directory where setup-envtest and binaries are stored (relative to project root) |
...
**Solution**: The script caches binaries in `ENVTEST_BIN_DIR` (default: `_output/tools/bin`). If the binaries already exist, subsequent runs will skip downloading. Consider caching this directory in CI.
